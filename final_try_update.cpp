...
class CtrlPacket{ 
 public:
   ...
   int mat;
   int act;
   CtrlPacket(int src_, int dst_, int mat_, int act_): src(src_), dst(dst_), mat(mat_), act(act_){}
   CtrlPacket(){}
};
class SDN_controller: public node {
 ...
 public:
   ...
   vector <CtrlPacket> ctrl_new_packet;
   vector <CtrlPacket> ctrl_del_packet;
   vector <CtrlPacket> ctrl_upd_packet;
   int ctrl_new_packet_cnt = 0;
   int ctrl_del_packet_cnt = 0;
   int ctrl_upd_packet_cnt = 0;
   int ctrl_new_packet_tot = 0;
   int stat_packet_cnt;
   int tot_node;
   int src, dst;
...
void ctrl_new_packet_event (unsigned int con_id, unsigned int id, unsigned int mat, unsigned int act, unsigned int t = event::getCurTime(), string msg = "default"){
  ...
  if ( id == BROCAST_ID || node::id_to_node(id) == nullptr ) {
    cerr << "new_id is incorrect" << endl; return;
  }
...
void ctrl_del_packet_event (unsigned int con_id, unsigned int id, unsigned int mat, unsigned int act, unsigned int t = event::getCurTime(), string msg = "default"){
  ...
  if ( id == BROCAST_ID || node::id_to_node(id) == nullptr ) {
    cerr << "del_id is incorrect" << endl; return;
  }
...
void ctrl_upd_packet_event (unsigned int con_id, unsigned int id, unsigned int mat, unsigned int act, unsigned int t = event::getCurTime(), string msg = "default"){
  ...
  if ( id == BROCAST_ID || node::id_to_node(id) == nullptr ) {
    cerr << "upd_id is incorrect" << endl; return;
  }
...
void SDN_switch::recv_handler (packet *p){
  ...
  if (p->type() == "SDN_data_packet") { // the switch receives a packet from the other switch   //ok
    ...
    SDN_ctrl_new_payload *l3 = dynamic_cast<SDN_ctrl_new_payload*> (p3->getPayload());
    SDN_ctrl_new_header *h3 = dynamic_cast<SDN_ctrl_new_header*> (p3->getHeader());
    //initial packet
    next_hop = l3->getActID();

    //if(l3->getMsg() == "second_new"){ //只有第二次new才ack，第一次不ack
    ...
    SDN_ctrl_upd_payload *l3 = dynamic_cast<SDN_ctrl_upd_payload*> (p3->getPayload());
    ...
    SDN_ctrl_del_payload *l3 = dynamic_cast<SDN_ctrl_del_payload*> (p3->getPayload());
...
void SDN_controller::recv_handler (packet *p){
  ...
  if(p->type() == "SDN_stat_packet"){
    ...
    if(stat_packet_cnt == 0){     //construct 所有的ctrl packet，同時送出new packet
      Bellman();

      for(idx=src; idx!=dst; idx=route_table[idx]){   //traverse new path. construcsend ctrl_new
        ...
        if(route_table[idx] != switch_ptr->origin_next_hop){
          if(switch_ptr->origin_next_hop != -1){  //原本有路，進行update
            ctrl_upd_packet[ctrl_upd_packet_cnt].dst = idx;
            ctrl_upd_packet[ctrl_upd_packet_cnt].act = route_table[idx];
            ctrl_upd_packet_cnt ++;
            //ctrl_upd_packet.push_back(CtrlPacket(getNodeID(), idx, dst, route_table[idx]));
            //ctrl_upd_packet_event(getNodeID(), idx, dst, route_table[idx], event::getCurTime());
          }
          else{ //原本沒路，進行new
            ctrl_new_packet[ctrl_new_packet_cnt].dst = idx;
            ctrl_new_packet[ctrl_new_packet_cnt].act = route_table[idx];
            ctrl_new_packet_cnt ++;   //紀錄送出了幾個new_packet，之後進行ack要用
            //ctrl_new_packet.push_back(CtrlPacket(getNodeID(), idx, dst, route_table[idx]));
            //ctrl_new_packet_event(getNodeID(), idx, dst, route_table[idx], event::getCurTime(), "second_new");    //直接送，不用存
          }
        }
      }
      ctrl_new_packet_tot = ctrl_new_packet_cnt;
      for(idx=src; idx!=dst; ){   //traverse old path. send del packet
        switch_ptr = dynamic_cast<SDN_switch*> (node::id_to_node(idx));
        ...
        if(route_table[idx] == -1){ //現在沒有路
          ...
          ctrl_del_packet[ctrl_del_packet_cnt].dst = idx;
          ctrl_del_packet[ctrl_del_packet_cnt].act = switch_ptr->origin_next_hop;
          ctrl_del_packet_cnt ++;
          //ctrl_del_packet.push_back(CtrlPacket(getNodeID(), idx, dst, switch_ptr->origin_next_hop));
      ...
      if(ctrl_new_packet_cnt == 0){ //沒有new _packet 直接update
        if(ctrl_upd_packet_cnt != 0){
                  if(ctrl_new_packet_cnt != 0){
                    exit(1);
                  }
          ctrl_upd_packet_event(getNodeID(), ctrl_upd_packet[ctrl_upd_packet_cnt-1].dst, dst, ctrl_upd_packet[ctrl_upd_packet_cnt-1].act, event::getCurTime());
        }
        else {  //也沒有update packet 直接del
                  if(ctrl_new_packet_cnt!=0 || ctrl_upd_packet_cnt!=0){
                    exit(1);
                  }
          for(int i=0; i<ctrl_del_packet_cnt; i++){
            ctrl_del_packet_event(getNodeID(), ctrl_del_packet[i].dst, dst, ctrl_del_packet[i].act, event::getCurTime());
          }
        }
      }
      else{ //現在有new packet全部送出去
        for(int i=0; i<ctrl_new_packet_tot; i++){
          ctrl_new_packet_event(getNodeID(), ctrl_new_packet[i].dst, dst, ctrl_new_packet[i].act, event::getCurTime(), "second_new");    //直接送，不用存
        }
      }
    }
  }
    ...
    SDN_ctrl_new_header *h3 = dynamic_cast<SDN_ctrl_new_header*> (p3->getHeader());
    SDN_ctrl_new_payload *l3 = dynamic_cast<SDN_ctrl_new_payload*> (p3->getPayload());
    if(h3->getDstID() == getNodeID()){    //ack, back from switch
      if(l3->getMsg() == "second_new"){   //更新用的new, 處理他!
        ctrl_new_packet_cnt --;
        if(ctrl_new_packet_cnt == 0){ //new_packet 送完了，開始送upd packet送，從new path裡最後更新的開始送
          if(ctrl_upd_packet_cnt != 0){   //確認是否有update packet
                  if(ctrl_new_packet_cnt != 0){
                    exit(1);
                  }
            ctrl_upd_packet_event(getNodeID(), ctrl_upd_packet[ctrl_upd_packet_cnt-1].dst, dst, ctrl_upd_packet[ctrl_upd_packet_cnt-1].act, event::getCurTime());
          }
          else{ //沒有update packet 直接送del
                  if(ctrl_new_packet_cnt!=0 || ctrl_upd_packet_cnt!=0){
                    exit(1);
                  }
            for(int i=0; i<ctrl_del_packet_cnt; i++){
    ...
    else{//get from ctrl_new_packet_event()
    ...
    SDN_ctrl_upd_packet *p3 = dynamic_cast<SDN_ctrl_upd_packet*> (p);
    SDN_ctrl_upd_header *h3 = dynamic_cast<SDN_ctrl_upd_header*> (p3->getHeader());
    if(h3->getDstID() == getNodeID()){    //ack, get from switch
      ctrl_upd_packet_cnt --;
      if(ctrl_upd_packet_cnt == 0){   //upd 全做完了，開始del
                  if(ctrl_new_packet_cnt!=0 || ctrl_upd_packet_cnt!=0){
                    exit(1);
                  }
        for(int i=0; i<ctrl_del_packet_cnt; i++){
    ...
    else{ //get rom ctrl_del_packet_event
      h3->setPreID(getNodeID());
      h3->setNexID(h3->getDstID());
      send_handler(p3);
    }
...
int main()
{
  ...
  CtrlPacket temp_for_decl_packet;
  ...
  controller->dis.assign(tot_node, UINT_MAX);
    controller->ctrl_upd_packet.assign(tot_node, temp_for_decl_packet);
    controller->ctrl_del_packet.assign(tot_node, temp_for_decl_packet);
    controller->ctrl_new_packet.assign(tot_node, temp_for_decl_packet);

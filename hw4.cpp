...
class SDN_stat_payload : public payload {
 ...
 public:
  ...
  void setInfo(map<int, int> _node_info){node_info = _node_info;}
  map<int, int> getInfo(){return node_info;}
 ...
 public:
   ...
   void Relax(int start, int end, int dest){		//start:start point(node) of a link, end:end point(node) of a link
     if(distance_[start] != UINT_MAX){
       unsigned int link_weight 					= node_adj_list_[start][end];
       unsigned int next_node_of_end  		= route_table_[end][dest];
       ...
       if((test_weight==distance_[end] && start<next_node_of_end) ||
         ...
         route_table_[end][dest] = start;
         distance_[end] = test_weight;
       }
     }
   }
   Graph(){}
   Graph(int nodes_cnt, int tot_dest_nodes){
     map<int, unsigned int> temp;
     route_table_.assign(nodes_cnt, temp);
     node_adj_list_.assign(nodes_cnt, temp);
  ...
  for(int dest=0; dest<dest_cnt; dest++){
    ...
    for(int k=0; k<node_cnt-1; k++){	//run at most n-1 times, because each time will find at least one more shortest path's link.
      for(int node=0; node<node_cnt; node++){	//each time, relax() every adjacent_node of every node
        for(iter=node_adj_list_[node].begin(); iter!=node_adj_list_[node].end(); iter++){	//對第j node之所有link relax
          adj_node = iter->first;
          this->Relax(node, adj_node, dest_[dest]);
        }
      }
    }
...
class SDN_controller: public node {
 ...
 public:
  ...
  map<unsigned int, vector<vector<unsigned int> > > round_packet;
  map<unsigned int, unsigned int> packet_alive_cnt;	//map<dest_id, alive_cnt;
  map<unsigned int, unsigned int> round_number;	//map<dest_id, alive_cnt;
  int stat_packet_now;
  ~SDN_controller(){}
...
void SDN_ctrl_pkt_gen_event::trigger() {
  ...
  hdr->setDstID(dst);
  hdr->setPreID(src);
  hdr->setNexID(src); // in hw3, you should set NexID to src //checked
  // payload
  pld->setMsg(msg);
  pld->setMatID(mat);
  pld->setActID(act);
  ...
  recv_event::recv_data e_data;
  e_data.s_id = src;
  e_data.r_id = src; // in hw3, you should set r_id it src	//checked
  e_data._pkt = pkt;
...
unsigned int SDN_stat_pkt_gen_event::event_priority() const {
  string string_for_hash;
  string_for_hash = to_string(getTriggerTime()) + to_string(src) + to_string(dst) + msg; //to_string (pkt->getPacketID());
  return get_hash_value(string_for_hash);
}
...
void stat_packet_event (unsigned int switch_id, unsigned int con_id, map<int, int> &node_info, unsigned int t = event::getCurTime(), string msg = "default"){
...
void SDN_controller::recv_handler (packet *p){
  ...
  if(p->type() == "SDN_ctrl_packet"){
    ...
    if(h3->getDstID() == getNodeID()){	
      ...
      if(packet_alive_cnt[match] == 0){	
        round_num = ++ round_number[match]; //move to next round
        packet_alive_cnt[match] = round_packet[match][round_num].size();  //get this round's packet_cnt
        if(packet_alive_cnt[match] != 0){
          //put all the packet's of this round to ctrl_packet_event();
          for(vector<unsigned int>::iterator it=round_packet[match][round_num].begin(); it!=round_packet[match][round_num].end(); it++){
            ctrl_packet_event(getNodeID(), *it, match, graph.route_table_[*it][match], event::getCurTime(), "update packet");	//route_table_[update][node_now][dst]
          }
        }
      }
    }
  ...
  else if(p->type() == "SDN_stat_packet"){  //stat_packet for update weight
    ...
    int src = h3->getSrcID();
    map<int, int> node_info = l3->getInfo();
      ...
      if(l3->getMsg() == "old"){  //finish install weight
        ...
        for(unsigned int i=0; i<tot_dest_nodes; i++){
          for(unsigned int j=0; j<tot_nodes; j++){
            ...
            if(destination != j){
              ctrl_packet_event(getNodeID(), j, destination, graph.route_table_[j][destination], event::getCurTime(), "original packet");
            }
          }
        }
      }
        ...
        for(unsigned int i=0; i<tot_dest_nodes; i++){
          ...
          round_packet[destination].push_back(expand);
          //prepare initial ctrl packets: add destination node's all previous nodes to this round
          for(vector<unsigned int>::iterator iter= graph.pre_nodes[destination][destination].begin(); iter!=graph.pre_nodes[destination][destination].end(); iter++){	
            round_packet[destination][0].push_back(*iter);	//add pre_node_now to this round
          }
          packet_alive_cnt[destination] = round_packet[destination][0].size();		//get packet_alive_cnt of first round
          //prepare round i's packets: traverse all the node in round i-1
          for(int i=1; round_packet[destination][i-1].size()>0; i++){
            round_packet[destination].push_back(expand);  //expand one more vector<unsigned int>
            for(vector<unsigned int>::iterator it=round_packet[destination][i-1].begin(); it!=round_packet[destination][i-1].end(); it++){	
              //add this node's all previous nodes to next round
              for(vector<unsigned int>::iterator iter= graph.pre_nodes[destination][*it].begin(); iter!=graph.pre_nodes[destination][*it].end(); iter++){	
                round_packet[destination][i].push_back(*iter);
              }
            }
          }
          //update real route table (send initial ctrl_packet);
          for(vector<unsigned int>::iterator it=round_packet[destination][0].begin(); it!=round_packet[destination][0].end(); it++){	//為這次要處理的node創造ctrl_packet
            ctrl_packet_event(getNodeID(), *it, destination, graph.route_table_[*it][destination], event::getCurTime(), "update packet");	//ctrl(con_id, dst, mat, act, upd_time);
          }
...
void SDN_switch::recv_handler (packet *p){
  ...
  else if (p->type() == "SDN_stat_packet") { // send stat_packet to controller
    SDN_stat_packet *p3 = dynamic_cast<SDN_stat_packet*> (p);
    SDN_stat_header *h3 = dynamic_cast<SDN_stat_header*> (p3->getHeader());
...
int main()
{
  ...
  vector<map<int, int> > temp_for_declear(tot_nodes);
  vector< vector<map<int, int> > > temp_weight;
  temp_weight.assign(2, temp_for_declear);
  ...
  controller->stat_packet_now = tot_nodes;
  for(int i=0; i<tot_nodes; i++){   //send node_i's old and new weight.
    stat_packet_event(i, con_id, temp_weight[OLD][i], ins_time, "old"); 
